...
function notConfigured<T>(reason: string): SourceResult<T> {
  return { state: "not_configured", note: reason, fetchedAt: null, items: [] };
}

async function guardedPull<T>(fn: () => Promise<T[]>): Promise<SourceResult<T>> {
  try {
    const items = await fn();
    return { state: "ok", note: null, fetchedAt: new Date().toISOString(), items };
  } catch (e) {
    if (e instanceof ErpNotConfiguredError || e instanceof ErpNotImplementedError) {
      return notConfigured(e.message);
    }
    return {
      state: "error",
...
export async function gatherEcnImpact(tenantId: string, ecnId: string): Promise<EcnImpact | null> {
  ...
  if (target.kind === "NONE") {
    const na = notConfigured<never>(target.reason);
    return {
      erpTarget: "NONE",
      affectedBoms,
      oldInventory: na,
  ...
  const [oldInventory, newInventory, openPo, excess, workOrders, salesOrders] = await Promise.all([
    ...
    guardedPull(async () =>
      ...
      // 如实给全量在制/计划工单让工程自行核对(注明口径),不假装能精确匹配
