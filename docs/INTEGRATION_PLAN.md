...
| # | 项 | 已有 | 缺 | 建议补入 | 记入日期 |
|---|---|---|---|---|---|
| B1 | **PM 采购申请单页** `/procurement/request`(整合方案 3.2) | `lib/domain/gtb.ts` 全部计算 + 12 单测;`PurchaseRequest` 表含 `gtbSnapshot`;同 MPN 呆滞数据在候选中已有 | PM 视角页面(录需求 → 展示 GTB 计算过程 → 提交采购) | PR8 或独立小 PR | 2026-07-27 |
| B2 | **供应商预设基础数据维护页**(整合方案 3.2) | `upsertSupplierOffer` 数据层已实现;`SupplierOffer`/`PriceBreak` 模型完整 | 维护界面(MOQ/LT/多阶价格的增删改查) | PR8 | 2026-07-27 |
| B3 | **采购异常阈值维护** | 领域函数 `evaluateFlags` 已参数化(价格线/交期线/币种) | 阈值当前是比价页内的演示常量(CNY 10 / 30 天),需维护界面;**阈值口径本身需业务确认** | PR8;口径确认前不得当作正式风控 | 2026-07-27 |
| B4 | 多源询价单次上限 20 个料号 | `truncated` 字段已如实返回 | 大批量分批(可复用 §15 ImportJob 框架) | 有真实大单需求时 | 2026-07-27 |
| B5 | 线下报价导入仅取 MPN + 单价列 | 落库字段 MOQ/SPQ/LT 已就绪 | 这三列的列映射与解析 | PR8 | 2026-07-27 |
