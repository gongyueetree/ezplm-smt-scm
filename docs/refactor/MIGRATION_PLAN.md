...
| **REF-0.5** | P0 修复 + 回归用例(R0-1…R0-6;R0-7 待商务) | REF-0 确认 | 低 | 无 |
...
1. 是否接受插入 **REF-0.5(P0 修复)** 与 **REF-0.8(对拍基建)**(§0 的两处偏差);
2. **R0-7(报价录入通道语义)** 属业务决策而非工程决策 —— 通用 Excel 录入的报价是否应参与比价?
3. `legacy-static/`(8.0MB)是归档移出仓库还是保留 —— 它是验收对照原件,建议**不删**。

确认后从 REF-0.5 开工;若确认维持原序列,则从 REF-1 开工,但 §0 的归因风险需书面接受。
