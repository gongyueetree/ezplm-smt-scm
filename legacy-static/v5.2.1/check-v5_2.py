...
退出码：0 全过 / 1 有失败。
"""
import os, re, sys, subprocess, shutil, math, json
...
# V5.2：行级模型——simulateReply 改 opoLines 后 renderOpo()，由派生函数统一重算
updates_abn = ('opoLines' in sr_body) or ('OPO.abnormal' in sr_body) or ('k-abn' in sr_body)
unified = 'renderOpo()' in sr_body
appends_rows = ('opoLines' in opo_t) and ('diff-body' in opo_t) and ('abn-body' in opo_t)
if not (updates_abn and appends_rows and unified):
    errors.append(f'opo.html simulateReply 未保持一致（改行级数据={updates_abn} 三表派生={appends_rows} 统一渲染={unified}）')
print(f'  simulateReply：改行级数据={updates_abn} · 三表派生自 opoLines={appends_rows} · 统一渲染={unified}')
...
checks_28 = {
    ...
    'PM 确认前校验异常已处理': bool(re.search(r"originalExceptions\(\)\.filter\(l=>!l\.purchaserDone\)", mqf)) or 'unresolvedCount' in mqf,
}
...
checks_30 = {
    '存在行级状态数组 opoLines[]': bool(re.search(r'const opoLines\s*=\s*\[', op)),
    '统一渲染 renderOpo()': 'function renderOpo()' in op and 'renderOpo();' in op,
    'KPI/三表派生自 opoLines': all(x in op for x in ['pendingLines','repliedLines','abnormalLines','pendingBySupplier']),
    '未回复供应商表动态渲染': 'noreply-body' in op and ('pendingBySupplier' in op),
    ...
    '差异/异常表无静态行（动态生成）': '<tbody id="diff-body"></tbody>' in op and '<tbody id="abn-body"></tbody>' in op,
}
...
# ================================================================
# 33–38：运行时行为断言（需 Node + jsdom）。区别于前面的静态/源码检查，
# 这些用无头 DOM 真正执行页面交互并断言结果，覆盖 GPT 第三轮诊断的 P0/P1。
# ================================================================
JSDOM_PATH = '/tmp/jsdom-env/node_modules/jsdom'
have_jsdom = node and os.path.exists(JSDOM_PATH)

def run_jsdom(label, page, body):
    """在 jsdom 中加载 page + contract.js，执行 body，body 通过 console.log('RESULT:'+json) 返回结果。"""
    harness = '''
const { JSDOM } = require(%r);
const fs = require('fs');
const dom = new JSDOM(fs.readFileSync(%r,'utf8'), {runScripts:'dangerously',resources:'usable',pretendToBeVisual:true});
const {document, window} = dom.window;
window.eval(fs.readFileSync(%r,'utf8'));
window.ezToast=()=>{}; window.ezLog=()=>{};
document.dispatchEvent(new window.Event('DOMContentLoaded'));
function modalBtn(i){const m=document.querySelector('.ez-modal-mask');m.querySelectorAll('[data-mbtn]')[i].click();}
try {
%s
} catch(e) { console.log('RESULT:'+JSON.stringify({__error:String(e)})); }
''' % (JSDOM_PATH, os.path.join(CC, page), os.path.join(CC, 'contract.js'), body)
    tmp = os.path.join(ROOT, '._jsdom_tmp.js')
    with open(tmp, 'w', encoding='utf-8') as f:
        f.write(harness)
    r = subprocess.run([node, tmp], capture_output=True, text=True)
    os.remove(tmp)
    ...
    for line in r.stdout.splitlines():
        if line.startswith('RESULT:'):
            try: out = json.loads(line[len('RESULT:'):])
            except: out = None
    return out

section('33–38. 运行时行为断言（jsdom）')
if not have_jsdom:
    warns.append('未安装 jsdom，跳过运行时断言（§33–§38）；建议安装后或用 Playwright 复验')
    print('  （未检测到 jsdom，跳过运行时断言）')
else:
    # ---- 33. 报价审批后参数冻结、导出用快照 ----
    r = run_jsdom('quote-freeze', 'quote.html', '''
['J2','T1'].forEach(ref=>{window.fixCls(ref);modalBtn(1);});
window.submitApprove(); modalBtn(1);
window.approveQuote(); modalBtn(1);
const approvedPrice = window.quoteRows()[1][9];
const discLocked = document.getElementById('disc').disabled;
document.getElementById('disc').value='80'; window.recalc();
const afterTamper = window.quoteRows()[1][9];
console.log('RESULT:'+JSON.stringify({discLocked, approvedPrice, afterTamper, frozen: approvedPrice===afterTamper}));
''')
    if not r or r.get('__error'):
        errors.append(f'§33 报价冻结断言执行失败: {r and r.get("__error")}')
    else:
        if not r.get('discLocked'): errors.append('§33 已批准后折扣输入未被冻结')
        if not r.get('frozen'): errors.append(f'§33 已批准导出价随实时输入漂移（{r.get("approvedPrice")}→{r.get("afterTamper")}）')
        print(f'  33 报价冻结: 折扣锁定={r.get("discLocked")} · 导出用快照不漂移={r.get("frozen")}（¥{r.get("approvedPrice")}）')

    # ---- 34. 报价退回后可创建修订版并重新提交 ----
    r = run_jsdom('quote-reject', 'quote.html', '''
['J2','T1'].forEach(ref=>{window.fixCls(ref);modalBtn(1);});
window.submitApprove(); modalBtn(1);
window.approveQuote();
const m=document.querySelector('.ez-modal-mask'); m.querySelector('#rej-reason').value='折扣超授权';
m.querySelectorAll('[data-mbtn]')[0].click();   // 退回
const statusAfterReject = document.getElementById('quote-status').textContent;
const reviseVisible = document.getElementById('btn-revise').style.display!=='none';
window.reviseQuote(); modalBtn(1);
const statusAfterRevise = document.getElementById('quote-status').textContent;
const canResubmit = document.getElementById('btn-submit').style.display!=='none';
console.log('RESULT:'+JSON.stringify({statusAfterReject,reviseVisible,statusAfterRevise,canResubmit}));
''')
    if not r or r.get('__error'):
        errors.append(f'§34 报价退回流程断言失败: {r and r.get("__error")}')
    else:
        if r.get('statusAfterReject')!='已退回': errors.append('§34 退回后状态非"已退回"')
        if not r.get('reviseVisible'): errors.append('§34 退回后无"创建修订版"入口（死路）')
        if r.get('statusAfterRevise')!='草稿' or not r.get('canResubmit'):
            errors.append('§34 创建修订版后未回到草稿/不可重新提交')
        print(f'  34 退回流程: 退回={r.get("statusAfterReject")} · 可修订={r.get("reviseVisible")} · 修订后={r.get("statusAfterRevise")} 可重提={r.get("canResubmit")}')

    # ---- 35. 单产线排产无时间重叠 + 16h/天 ----
    r = run_jsdom('schedule-overlap', 'planning.html', '''
const rows=window.schedule();
let overlap=false, prev=null;
rows.forEach(x=>{ if(prev && x.startDT < prev) overlap=true; prev=x.finDT; });
// 校验 16h/天：取最大工时工单，自然时长应 >= 工时（含夜间停工）
const big=rows.reduce((a,b)=>b.hours>a.hours?b:a,rows[0]);
const wall=(new Date(big.finDT)-new Date(big.startDT))/3600000;
console.log('RESULT:'+JSON.stringify({overlap, seqCount:rows.length, allSeq:rows.every((x,i)=>x.seq===i+1), wallOK: wall>=big.hours}));
''')
    if not r or r.get('__error'):
        errors.append(f'§35 排产断言执行失败: {r and r.get("__error")}')
    else:
        if r.get('overlap'): errors.append('§35 单产线排产存在时间重叠（下一工单早于上一完成）')
        if not r.get('wallOK'): errors.append('§35 16h/天工作日历未正确跨天（自然时长 < 工时）')
        if not r.get('allSeq'): errors.append('§35 并非所有工单都有连续顺位')
        print(f'  35 排产: 无重叠={not r.get("overlap")} · 全部有顺位={r.get("allSeq")}（{r.get("seqCount")} 行）· 跨天正确={r.get("wallOK")}')

    # ---- 36. completedOps 真正影响排产工时 ----
    r = run_jsdom('schedule-ops', 'planning.html', '''
const before=window.schedule().find(x=>x.w.wo==='WO-7752');
const ratio = before.hours/before.fullHours;   // completedOps=1, 3 工序 → 应≈2/3
console.log('RESULT:'+JSON.stringify({hours:before.hours, full:before.fullHours, opsAffects: Math.abs(ratio-2/3)<0.05}));
''')
    if not r or r.get('__error'):
        errors.append(f'§36 工序排产断言失败: {r and r.get("__error")}')
    else:
        if not r.get('opsAffects'): errors.append('§36 completedOps 未真正影响排产工时（仍是展示用）')
        print(f'  36 工序参与排产: 排产 {r.get("hours")}h / 总 {r.get("full")}h · 工序生效={r.get("opsAffects")}')

    # ---- 37. OPO 行级模型：各表/ KPI 一致 + 门户提交移除异常 ----
    r = run_jsdom('opo-consistency', 'opo.html', '''
function snap(){return {
  pending:+document.getElementById('k-pending').textContent,
  replied:+document.getElementById('k-replied').textContent,
  abn:+document.getElementById('k-abn').textContent,
  diff:document.querySelectorAll('#diff-body tr').length,
  abnRows:document.querySelectorAll('#abn-body tr').length,
  supSum:[...document.querySelectorAll('#noreply-body tr')].reduce((a,tr)=>a+ +tr.children[1].textContent,0),
  has5113:[...document.querySelectorAll('#abn-body tr')].some(tr=>tr.textContent.includes('OPO-5113-02'))
};}
const a=snap();
document.getElementById('pr-eta').value='2026-06-28'; window.portalSubmit();
const b=snap();
window.simulateReply();
const c=snap();
const consistent = [a,b,c].every(s=> s.pending===s.supSum && s.replied===s.diff && s.abn===s.abnRows);
console.log('RESULT:'+JSON.stringify({consistent, initHas5113:a.has5113, afterPortalHas5113:b.has5113, seq:[a,b,c].map(s=>[s.pending,s.replied,s.abn])}));
''')
    if not r or r.get('__error'):
        errors.append(f'§37 OPO 一致性断言失败: {r and r.get("__error")}')
    else:
        if not r.get('consistent'): errors.append(f'§37 OPO KPI 与三表不一致: {r.get("seq")}')
        if not r.get('initHas5113'): errors.append('§37 初始异常表应含 OPO-5113-02（超期未回复）')
        if r.get('afterPortalHas5113'): errors.append('§37 门户提交后 OPO-5113-02 仍在异常表（未据回复移除）')
        print(f'  37 OPO 行级一致: 全程一致={r.get("consistent")} · 门户提交后异常移除={not r.get("afterPortalHas5113")}')

    # ---- 38. 物料报价：新价输入即时显示 + 全部原始异常获 PM 确认 + 换货源记新供应商 ----
    r = run_jsdom('mq-requote', 'material-quote.html', '''
const L0src = null;
const btn=()=>document.getElementById('next-btn');
btn().click();btn().click();btn().click();btn().click();
const npFirst=document.querySelectorAll('.ez-modal-mask [id^="np-"]').length;
function setSel(mpn,val){const s=document.querySelector('.ez-modal-mask #dec-'+mpn);s.value=val;s.dispatchEvent(new window.Event('change'));}
setSel('GRM188R71H104KA93D','REQUOTE');
const npEnabledNow=(document.querySelector('.ez-modal-mask #np-GRM188R71H104KA93D')||{}).disabled===false;
setSel('STM32F103C8T6','ACCEPT');setSel('PESD5V0S1BA','REPLACE_SUPPLIER');
let m=document.querySelector('.ez-modal-mask');
m.querySelector('#np-GRM188R71H104KA93D').value='0.0175';
m.querySelector('#np-PESD5V0S1BA').value='0.105';
m.querySelector('#ns-PESD5V0S1BA').value='新光电子'; m.querySelector('#lt-PESD5V0S1BA').value='21';
m.querySelectorAll('[data-mbtn]')[1].click();   // 保存
btn().click();   // PM 确认
// 读取 L（通过结果表导出反推确认人/供应商）
window.ezExportCSV=(name,h,rows)=>{ window.__rows=rows; };
btn().click();btn().click();btn().click();   // 提交审批→通过→下载
const rows=window.__rows||[];
const idx={mpn:0, conf:rows[0]?undefined:undefined};
// 列：MPN..处理结论(11) 新供应商(12) LT(13) 处理人(14) 确认人(15)
const all=rows.filter(r=>['STM32F103C8T6','GRM188R71H104KA93D','PESD5V0S1BA'].includes(r[0]));
const allConfirmed=all.length===3 && all.every(r=>r[15]==='王 PM');
const pesd=all.find(r=>r[0]==='PESD5V0S1BA');
console.log('RESULT:'+JSON.stringify({npFirst, npEnabledNow, allConfirmed, pesdSupplier:pesd?pesd[12]:null, pesdLT:pesd?pesd[13]:null}));
''')
    if not r or r.get('__error'):
        errors.append(f'§38 物料报价断言失败: {r and r.get("__error")}')
    else:
        if r.get('npFirst',0) < 3: errors.append('§38 采购处理弹窗首次打开未渲染全部新价输入框')
        if not r.get('npEnabledNow'): errors.append('§38 选 REQUOTE 后新价输入未即时启用')
        if not r.get('allConfirmed'): errors.append('§38 并非所有原始异常行都获 PM 确认')
        if r.get('pesdSupplier')!='新光电子': errors.append('§38 更换货源未记录新供应商')
        print(f'  38 物料报价: 新价输入即时={r.get("npEnabledNow")} · 全原始异常确认={r.get("allConfirmed")} · 换货源记供应商={r.get("pesdSupplier")}(LT {r.get("pesdLT")})')
