...
gate_ok = ('purchaserDone' in mq) and bool(re.search(r'filter\(l=>!l\.purchaserDone\)', mq)) or ("every(l=>l.decision)" in mq.replace(' ',''))
if not gate_ok:
    errors.append('material-quote.html 未要求所有异常处理完毕后才可提交')
print('  逐项确认/处理门禁检查完成')
...
if not (updates_abn and appends_rows):
    errors.append(f'opo.html simulateReply 未保持一致（更新异常={updates_abn} 追加差异/异常行={appends_rows}）')
print(f'  simulateReply：更新异常={updates_abn} · 追加差异/异常行={appends_rows}')
